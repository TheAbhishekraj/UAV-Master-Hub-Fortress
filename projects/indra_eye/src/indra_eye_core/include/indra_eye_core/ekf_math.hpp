...
inline Quaterniond rotationVectorToQuaternion(const Vector3d &rot_vec) {
  ...
  if (angle < 1e-8) {
    ...
    return Quaterniond(1.0, 0.5 * rot_vec(0), 0.5 * rot_vec(1),
                       0.5 * rot_vec(2));
  } else {
    // Exact formula
    Vector3d axis = rot_vec / angle;
    double half_angle = 0.5 * angle;
    return Quaterniond(std::cos(half_angle), std::sin(half_angle) * axis(0),
 ...
 */
inline Matrix15d computeStateTransitionMatrix(const Quaterniond &orientation,
  ...
  Phi.block<3, 3>(0, 6) =
      -R * accel_skew * dt2_half;        // δp += -R*[a]× * δθ * dt²/2
 ...
 */
inline Matrix15d
computeProcessNoiseCovariance(const Quaterniond &orientation,
  ...
  double accel_var =
      noise_params.accel_noise_density * noise_params.accel_noise_density;
  Q.block<3, 3>(3, 3) =
      R * Matrix3d::Identity() * R.transpose() * accel_var * dt;

  // Gyroscope noise contribution to orientation error
  double gyro_var =
      noise_params.gyro_noise_density * noise_params.gyro_noise_density;
  Q.block<3, 3>(6, 6) =
      R * Matrix3d::Identity() * R.transpose() * gyro_var * dt;

  // Accelerometer bias random walk
  double accel_rw_var =
      noise_params.accel_random_walk * noise_params.accel_random_walk;
  Q.block<3, 3>(9, 9) = Matrix3d::Identity() * accel_rw_var * dt;

  // Gyroscope bias random walk
  double gyro_rw_var =
      noise_params.gyro_random_walk * noise_params.gyro_random_walk;
 ...
 */
inline void propagateNominalState(ESEKFState &state, const IMUMeasurement &imu,
                                  double dt = DT) {
  ...
  state.position +=
      state.velocity * dt + 0.5 * (R * accel_corrected + gravity) * dt * dt;
 ...
 */
inline void propagateErrorCovariance(ESEKFState &state,
                                     const Vector3d &accel_measurement,
                                     const IMUNoiseParams &noise_params,
                                     double dt = DT) {

  Matrix15d Phi =
      computeStateTransitionMatrix(state.orientation, accel_measurement, dt);
  Matrix15d Q =
      computeProcessNoiseCovariance(state.orientation, noise_params, dt);
 ...
 *
 * After measurement update, the error state is injected back into the nominal
 * state: p ← p + δp v ← v + δv q ← q ⊗ [1, δθ/2]^T b_a ← b_a + δb_a b_g ← b_g +
 * δb_g
 *
