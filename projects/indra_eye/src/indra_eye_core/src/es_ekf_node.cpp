...
public:
  ESEKFNode() : Node("es_ekf_node") {
    ...
    this->declare_parameter("visual_weight", 1.0);
    // Topic name parameters — allows remapping without recompilation
    // PX4 XRCE-DDS bridge publishes IMU on fmu/out/sensor_combined
    // which is bridged as sensor_msgs/msg/Imu via DDS type adaptation
    this->declare_parameter("imu_topic", "/fmu/out/sensor_combined");
    this->declare_parameter("gnss_topic", "/fmu/out/vehicle_gps_position");

    // Subscribers — topics resolved from parameters for flexibility
    auto imu_topic = this->get_parameter("imu_topic").as_string();
    auto gnss_topic = this->get_parameter("gnss_topic").as_string();

    // PX4 XRCE-DDS publishes with BEST_EFFORT + TRANSIENT_LOCAL.
    // A VOLATILE subscriber silently receives nothing — QoS must match.
...
private:
  void imuCallback(const sensor_msgs::msg::Imu::SharedPtr msg) {
    ...
    IMUMeasurement imu;
    imu.accel = Vector3d(msg->linear_acceleration.x, msg->linear_acceleration.y,
                         msg->linear_acceleration.z);
    imu.gyro = Vector3d(msg->angular_velocity.x, msg->angular_velocity.y,
  ...
  void gnssCallback(const sensor_msgs::msg::NavSatFix::SharedPtr msg) {
    ...
    Vector3d gnss_position(msg->longitude * 111320.0 *
                               std::cos(msg->latitude * M_PI / 180.0),
                           msg->latitude * 111320.0, msg->altitude);
  ...
  void vioCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    Vector3d vio_position(msg->pose.pose.position.x, msg->pose.pose.position.y,
                          msg->pose.pose.position.z);

    // Extract covariance and apply visual_weight
    double visual_weight = this->get_parameter("visual_weight").as_double();
    Vector3d vio_cov(msg->pose.covariance[0] / visual_weight,
                     msg->pose.covariance[7] / visual_weight,
                     msg->pose.covariance[14] / visual_weight);
  ...
  void slamCallback(
    ...
    Vector3d slam_position(msg->pose.pose.position.x, msg->pose.pose.position.y,
                           msg->pose.pose.position.z);

    // Extract covariance
    Vector3d slam_cov(msg->pose.covariance[0], msg->pose.covariance[7],
  ...
  void updateWithPositionMeasurement(const Vector3d &measured_pos,
    ...
    Eigen::Matrix<double, 15, 3> K =
        state_->covariance * H.transpose() * S.inverse();
    ...
    state_->covariance =
        I_KH * state_->covariance * I_KH.transpose() +
  ...
  void publishDiagnostics() {
    ...
    double pos_uncertainty =
        std::sqrt(state_->covariance(0, 0) + state_->covariance(1, 1) +
